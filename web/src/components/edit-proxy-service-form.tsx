...
export function EditProxyServiceForm({ listener, group, nodes, subscriptions, onSaved, onCancel, onNotice }: { listener: ListenerRecord; group: ProxyGroup; nodes: NodeRecord[]; subscriptions: Subscription[]; onSaved: () => Promise<void>; onCancel: () => void; onNotice: Notice }) {
  ...
  const [wsPath, setWSPath] = useState(listener.transport?.ws_path ?? "/hx-proxy")
  ...
  const valid = name.trim() && port > 0 && port < 65536 && (mode === "direct" || (mode === "rule" ? subscriptionIDs.length : nodeIDs.length)) && (!advanced || (publicHost.trim() && wsPath.startsWith("/"))) && (!replaceAuth || (username.trim() && password))
  ...
  async function save() {
    setSaving(true)
    let updatedGroup: ProxyGroup | null = null
    try {
      updatedGroup = await api.updateProxyGroup(group.id, {
      ...
      await api.updateListener(listener.id, {
        ...
        name: `${name.trim()} 入口`, kind, bind_address: advanced ? "127.0.0.1" : bindAddress.trim(), port,
      ...
      if (updatedGroup) {
        await api.updateProxyGroup(group.id, { version: updatedGroup.version, name: group.name, strategy: group.strategy, source_spec: group.source_spec, enabled: group.enabled, empty_behavior: group.empty_behavior, fallback_target_id: group.fallback_target_id }).catch(() => undefined)
      }
  ...
  return <div className="space-y-4 p-4">
    ...
    <div className="border-t pt-4"><div className="mb-3 text-xs font-semibold">入口配置</div><div className="grid gap-3 md:grid-cols-4"><Field label="协议"><Select value={kind} onValueChange={(value) => setKind(value as ListenerKind)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{["mixed", "http", "socks", "vless", "vmess", "trojan"].map((value) => <SelectItem key={value} value={value}>{value.toUpperCase()}{["vless", "vmess", "trojan"].includes(value) ? " WS" : ""}</SelectItem>)}</SelectContent></Select></Field><Field label="绑定 IP"><Input value={advanced ? "127.0.0.1" : bindAddress} onChange={(event) => setBindAddress(event.target.value)} disabled={advanced} /></Field><Field label="端口"><Input type="number" min={1} max={65535} value={port} onChange={(event) => setPort(Number(event.target.value))} /></Field><label className="flex h-8 items-center gap-2 self-end rounded-md border bg-white px-3 text-xs"><Checkbox checked={enabled} onCheckedChange={(value) => setEnabled(value === true)} />启用入口</label></div></div>
    {advanced && <div className="grid gap-3 md:grid-cols-2"><Field label="Cloudflare / 雷池域名"><Input value={publicHost} onChange={(event) => setPublicHost(event.target.value)} /></Field><Field label="WebSocket Path"><Input value={wsPath} onChange={(event) => setWSPath(event.target.value)} /></Field></div>}
    <div className="grid gap-3 md:grid-cols-2"><label className="flex items-center gap-2 rounded-md border bg-white px-3 py-2 text-xs"><Checkbox checked={replaceAuth} onCheckedChange={(value) => { setReplaceAuth(value === true); if (value) setClearAuth(false) }} />设置新凭据</label><label className="flex items-center gap-2 rounded-md border bg-white px-3 py-2 text-xs"><Checkbox checked={clearAuth} onCheckedChange={(value) => { setClearAuth(value === true); if (value) setReplaceAuth(false) }} disabled={advanced} />清除现有认证</label></div>
