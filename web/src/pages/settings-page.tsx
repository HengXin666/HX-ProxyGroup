        ...
        <TabsContent value="runtime">
          <SettingsPanel icon={Settings2} title="性能与运行" description="这些选项会影响 Mihomo 的连接与日志行为。">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <Toggle label="共享入口（推荐）" checked={settings.shared_inbound.mode === "shared"} onChange={(enabled) => setSettings({ ...settings, shared_inbound: { ...settings.shared_inbound, mode: enabled ? "shared" : "per_service", mixed_port: settings.shared_inbound.mixed_port || 7890, ws_port: settings.shared_inbound.ws_port || 7891, mixed_bind_address: settings.shared_inbound.mixed_bind_address || "0.0.0.0" } })} />
              <Field label="共享 Mixed 端口"><Input type="number" min={1} max={65535} disabled={settings.shared_inbound.mode !== "shared"} value={settings.shared_inbound.mixed_port ?? 7890} onChange={(event) => setSettings({ ...settings, shared_inbound: { ...settings.shared_inbound, mixed_port: Number(event.target.value) } })} /></Field>
              <Field label="共享 WS 内部端口"><Input type="number" min={1} max={65535} disabled={settings.shared_inbound.mode !== "shared"} value={settings.shared_inbound.ws_port ?? 7891} onChange={(event) => setSettings({ ...settings, shared_inbound: { ...settings.shared_inbound, ws_port: Number(event.target.value) } })} /></Field>
              <Field label="共享 Mixed 绑定 IP"><Input disabled={settings.shared_inbound.mode !== "shared"} value={settings.shared_inbound.mixed_bind_address ?? "0.0.0.0"} onChange={(event) => setSettings({ ...settings, shared_inbound: { ...settings.shared_inbound, mixed_bind_address: event.target.value } })} /></Field>
              <Field label="共享 WS 公网域名（反代）"><Input disabled={settings.shared_inbound.mode !== "shared"} placeholder="ws.example.com" value={settings.shared_inbound.ws_public_host ?? ""} onChange={(event) => setSettings({ ...settings, shared_inbound: { ...settings.shared_inbound, ws_public_host: event.target.value } })} /></Field>
              <Field label="共享 WS 公网端口"><Input type="number" min={1} max={65535} disabled={settings.shared_inbound.mode !== "shared"} value={settings.shared_inbound.ws_public_port ?? 443} onChange={(event) => setSettings({ ...settings, shared_inbound: { ...settings.shared_inbound, ws_public_port: Number(event.target.value) } })} /></Field>
              <Field label="共享 Mixed 公网主机名 / IP（可选）"><Input disabled={settings.shared_inbound.mode !== "shared"} placeholder="VPS 公网地址" value={settings.shared_inbound.mixed_public_host ?? ""} onChange={(event) => setSettings({ ...settings, shared_inbound: { ...settings.shared_inbound, mixed_public_host: event.target.value } })} /></Field>
              <Field label="共享 Mixed 公网端口"><Input type="number" min={1} max={65535} disabled={settings.shared_inbound.mode !== "shared"} value={settings.shared_inbound.mixed_public_port ?? settings.shared_inbound.mixed_port ?? 7890} onChange={(event) => setSettings({ ...settings, shared_inbound: { ...settings.shared_inbound, mixed_public_port: Number(event.target.value) } })} /></Field>
            </div>
            <p className="mt-3 text-xs leading-5 text-muted-foreground">共享入口开启后，所有 HTTP / SOCKS5 / Mixed 服务复用同一个端口，所有 VLESS / VMess / Trojan 服务复用同一个内部端口，全部由 Mihomo 按用户名（IN-USER）分流。这样只需要 1 个客户端进程、2 个 URL 就能同时使用所有代理组与协议，不再需要为每个组单独绑定端口。关闭后恢复“每个服务一个端口”的旧行为。</p>
            <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Toggle label="TCP 并发建连" checked={settings.performance.tcp_concurrent} onChange={(tcp_concurrent) => setSettings({ ...settings, performance: { ...settings.performance, tcp_concurrent } })} /><Toggle label="统一延迟计算" checked={settings.performance.unified_delay} onChange={(unified_delay) => setSettings({ ...settings, performance: { ...settings.performance, unified_delay } })} /><Field label="Keepalive 空闲（秒）"><Input type="number" min={0} max={600} value={settings.performance.keep_alive_idle_seconds} onChange={(event) => setSettings({ ...settings, performance: { ...settings.performance, keep_alive_idle_seconds: Number(event.target.value) } })} /></Field><Field label="Keepalive 间隔（秒）"><Input type="number" min={0} max={600} value={settings.performance.keep_alive_interval_seconds} onChange={(event) => setSettings({ ...settings, performance: { ...settings.performance, keep_alive_interval_seconds: Number(event.target.value) } })} /></Field><Field label="进程识别"><Select value={settings.performance.find_process_mode} onValueChange={(value) => setSettings({ ...settings, performance: { ...settings.performance, find_process_mode: value as GlobalSettings["performance"]["find_process_mode"] } })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="off">关闭</SelectItem><SelectItem value="strict">严格</SelectItem><SelectItem value="always">始终</SelectItem></SelectContent></Select></Field><Field label="数据面日志"><Select value={settings.performance.log_level} onValueChange={(value) => setSettings({ ...settings, performance: { ...settings.performance, log_level: value as GlobalSettings["performance"]["log_level"] } })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{["silent", "error", "warning", "info", "debug"].map((value) => <SelectItem key={value} value={value}>{value}</SelectItem>)}</SelectContent></Select></Field></div>
          </SettingsPanel>
        </TabsContent>
